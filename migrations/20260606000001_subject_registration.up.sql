-- Layer 7: Support SubjectCaptured / SubjectBound event projections.
--
-- Changes:
-- 1. Index on SubjectCaptured events for find_journeys_by_subject queries.
...
-- 4. Backfill: synthesise a SubjectCaptured-style lookup for existing subjects
...
CREATE INDEX idx_events_subject_captured_subject
    ON events ((payload -> 'SubjectCaptured' ->> 'subject_id'))
    WHERE event_type = 'SubjectCaptured';
...
COMMENT ON TABLE journey_person IS
    ...
    'PersonDetailsUpdated, SubjectCaptured (email updates). '
