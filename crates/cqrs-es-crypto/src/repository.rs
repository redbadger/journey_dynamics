...
mod tests {
    ...
    // ── Transactional path (postgres feature) ──────────────────────────────────
    ...
    mod postgres {
        ...
        use crate::{
            cipher::FieldCipher,
            kek::StaticKekProvider,
            key_store::{KeyStore, PostgresKeyStore},
            repository::PersistHook,
        };

        use super::{
            CryptoShreddingEventRepository, InMemoryEventRepository, TestAggregate, TestPiiCodec,
            pii_event, plain_event,
        };
        ...
        async fn dek_exists(pool: &sqlx::Pool<sqlx::Postgres>, subject_id: &Uuid) -> bool {
            let count: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM subject_encryption_keys WHERE subject_id = $1",
            )
            .bind(subject_id)
            .fetch_one(pool)
            .await
        ...
        async fn test_hook_receives_unencrypted_events() {
            ...
            struct CapturingHook {
                captured: Arc<Mutex<Vec<cqrs_es::persist::SerializedEvent>>>,
            }
        ...
        async fn test_hook_error_rolls_back_entire_transaction() {
            ...
            impl PersistHook for FailingHook {
                async fn on_persist(
                    &self,
                    _events: &[cqrs_es::persist::SerializedEvent],
                    _tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
                ) -> Result<(), PersistenceError> {
        ...
        async fn test_get_or_create_key_in_tx_reuses_dek_within_batch() {
            ...
            let dek_count: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM subject_encryption_keys WHERE subject_id = $1",
            )
            .bind(subject_id)
            .fetch_one(&pool)
