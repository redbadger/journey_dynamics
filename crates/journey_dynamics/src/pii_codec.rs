...
//!
//! The Journey aggregate's only encrypted event is the path-keyed
//! `AttributesSet`, whose contents are domain-agnostic, so the codec is the
//! generic [`AttributesSetCodec`](es_capture::attributes_set_codec::AttributesSetCodec)
//! from the foundation, re-exported here under the historical name
//! `JourneyPiiCodec`.

pub use es_capture::attributes_set_codec::AttributesSetCodec as JourneyPiiCodec;
