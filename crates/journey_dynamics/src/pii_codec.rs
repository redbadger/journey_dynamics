...
mod tests {
    ...
    // ── Legacy on-disk format (pre-partition back-compat) ────────────────────────

    // These tests exercise the `extract_encrypted_legacy` path generated by
    // `#[derive(PiiCodec)]` for events written before the partitioned-ciphertext
    // format was introduced.  The fixture payloads use the old sentinel field
    // names (`encrypted_pii`, `encrypted_data`) and the old AAD scheme
    // (`"aggregate_id:sequence"`) so the repository can decrypt them despite
    // never having written them through the new path.

    #[tokio::test]
    async fn test_legacy_person_captured_decrypts_via_back_compat() {
        let (repo, key_store) = make_repo_with_parts();
        let aggregate_id = "journey-legacy-pc-compat";
        let subject_id = Uuid::new_v4();

        // Create a DEK and encrypt with the old AAD scheme.
        let dek = key_store.get_or_create_key(&subject_id).await.unwrap();
        let cipher = FieldCipher::new();
        // Old `classify_arm` for multi-secret bundles all secret fields into one
        ...
        // Construct old-format payload (encrypted_pii + nonce, no partitions).
        let legacy_payload = serde_json::json!({
            "PersonCaptured": {
                "person_ref":    "passenger_0",
                "subject_id":    subject_id.to_string(),
        ...
        assert!(
            pc.get("encrypted_pii").is_none(),
            "sentinel must be absent after decryption"
        );
    }
    ...
    async fn test_legacy_person_captured_redacts_when_key_deleted() {
        let (repo, key_store) = make_repo_with_parts();
        let aggregate_id = "journey-legacy-pc-redact-compat";
        let subject_id = Uuid::new_v4();
        ...
        assert_eq!(pc["email"].as_str().unwrap(), "[redacted]");
        assert!(pc["phone"].is_null(), "phone must be null after shredding");
        // Non-PII fields survive shredding.
        assert_eq!(pc["person_ref"].as_str().unwrap(), "passenger_0");
        assert_eq!(
            pc["subject_id"].as_str().unwrap(),
            subject_id.to_string().as_str()
    ...
    async fn test_legacy_person_details_updated_decrypts_via_back_compat() {
        let (repo, key_store) = make_repo_with_parts();
        let aggregate_id = "journey-legacy-pd-compat";
        let subject_id = Uuid::new_v4();

        // Old `classify_arm` for single-secret uses the field value directly
        // (no wrapping object) as `plaintext_pii`.
        let dek = key_store.get_or_create_key(&subject_id).await.unwrap();
        let cipher = FieldCipher::new();
        let data_value = serde_json::json!({
            "passportNumber": "GB123456789",
            "dateOfBirth":    "1990-05-15",
            "nationality":    "GB",
        });
        let plaintext = serde_json::to_vec(&data_value).unwrap();
        let old_aad = format!("{aggregate_id}:1").into_bytes();
        let encrypted = cipher.encrypt(&dek, &plaintext, &old_aad);
    ...
    async fn test_legacy_person_details_updated_redacts_when_key_deleted() {
        let (repo, key_store) = make_repo_with_parts();
        let aggregate_id = "journey-legacy-pd-redact-compat";
        let subject_id = Uuid::new_v4();
        ...
        assert_eq!(
            pd["data"],
            serde_json::json!({}),
            "data must be redacted to empty object after shredding"
        );
        assert_eq!(pd["person_ref"].as_str().unwrap(), "passenger_0");
        assert_eq!(
            pd["subject_id"].as_str().unwrap(),
